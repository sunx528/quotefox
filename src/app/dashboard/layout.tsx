...
import { LayoutGrid, Users2, BarChart3, CreditCard, LogOut } from "lucide-react";
    ...
    <div className="flex min-h-full">
        ...
        <nav className="flex-1 space-y-1 px-3">
          {NAV.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-muted hover:bg-black/[0.04] hover:text-foreground dark:hover:bg-white/[0.06]"
            >
        ...
        <div className="border-t border-border p-4">
          <div className="flex items-center justify-between">
            <div className="min-w-0">
              ...
              <Badge tone="brand" className="mt-1">Forfait {label(PLAN_LABELS, sub?.plan ?? "free")}</Badge>
            </div>
          </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <nav className="flex gap-1 overflow-x-auto border-b border-border px-3 py-2 sm:hidden">
          {NAV.map((item) => (
            <Link key={item.href} href={item.href} className="shrink-0 rounded-lg px-3 py-1.5 text-sm font-medium text-muted hover:bg-black/[0.04]">
              {item.label}
            </Link>
          ))}
        </nav>
